...
public class ExecutorSeparator {
    ...
    // 内部类
    class DoSeparator implements Callable<String[][]> {
        private String[] arr_string;
        private int start;
        private int end;


        public DoSeparator(final String[] arr_string, int start, int end) {
        ...
        public String[][] call() throws Exception {
            ...
            for (int i = start; i < end; i++) {
                temp[i] = thulac.run(arr_string[i]);
            }
            return temp;
        }
    }
    ...
    public String[][] separate(final String[] arr_string) {
        ...
        for (int i = 0; i < cpuCoreNumber; i++) {
            ...
            //计算数据中的长度,避免数组长度不足10000
            int length = -1;
            if(arr_string[9999]==null) {
                for (int j = arr_string.length - 1; j >= 0; j--) {
            ...
            int increment = length / cpuCoreNumber + 1;
            int start = increment * i;
            int end = increment * i + increment;
            if (end > arr_string.length)
                end = arr_string.length;
            DoSeparator subCalc = new DoSeparator(arr_string, start, end);
